...
def prefilter_boxes(boxes, scores, labels, weights, thr):
    ...
    for t in range(len(boxes)):
        ...
        for j in range(len(boxes[t])):
            ...
            b = [int(label), float(score) * weights[t], x1, y1, x2, y2, x3, y3, x4, y4]
...
def get_weighted_box(boxes):
    ...
    conf = 0
    for b in boxes:
        iou = quadrangle_intersection_over_union(b[2:], best_box[2:])
        weight = b[1] * iou
        box[2:] += (weight * b[2:])
        conf += weight
    box[0] = best_box[0]
...
def find_matching_box(boxes_list, new_box, match_iou):
    ...
    for i in range(len(boxes_list)):
        ...
        if box[0] != new_box[0]:
            continue
        iou = quadrangle_intersection_over_union(box[2:], new_box[2:])
        if iou > best_iou:
            best_index = i
            best_iou = iou
